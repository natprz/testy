...
class Transformacje:
    ...
    def plh2gk(self, phi, lam, lam0=21):
        ...
        XGK = si + ((d_lam ** 2) / 2) * N * np.sin(phi) * np.cos(phi) * (
                1 + ((d_lam ** 2) / 12) * np.cos(phi) ** 2 * (5 - t ** 2 + 9 * eta2 + 4 * eta2 ** 2) + (
                (d_lam ** 4) / 360) * np.cos(phi) ** 4 * (
                        61 - 18 * t ** 2 + t ** 4 + 270 * eta2 - 330 * eta2 * t ** 2))
        YGK = d_lam * N * np.cos(phi) * (
                1 + d_lam ** 2 / 6 * np.cos(phi) ** 2 * (1 - t ** 2 + eta2) + d_lam ** 4 / 120 * np.cos(
            phi) ** 4 * (5 - 18 * t ** 2 + t ** 4 + 14 * eta2 - 58 * eta2 * t ** 2))
        return (XGK, YGK)
    ...
    def wczytaj_dane(self, plik):
        ...
        for i in range(0, len(X)):
            ...
            x[i], y[i], z[i] = self.plh2xyz(phi[i], lam[i], hel[i])
        neu = self.neu(X, Y, Z)
        phi, lam = np.rad2deg(phi), np.rad2deg(lam)
        return X, Y, Z, phi, lam, hel, x2, y2, x92, y92, x, y, z, neu

    def uruchom(self, X, Y, Z, phi, lam, hel, x2, y2, x92, y92, x, y, z, neu):
        plik = open('wsp_out.txt', 'w')
        plik.write(
            f'Transformacje współrzędnych \nAutor:Natalia Przygoda\nPoniżej zestawienie współrzędnych podanych punktów\n')
        plik.write(f'{"=" * 60}\n|X\t\t|Y\t\t|Z\t\t|\n')
        for i in range(0, len(X)):
